...
#'   and it contains row names, they will be used as feature IDs.
...
#'   column named as `FeatureID` that contains feature identifiers as character 
#'   strings. In case the input table contains the column `FeatureID`, the
...
readFeatureAnnotationFile <- function(file, ...) {
  ...
  if("FeatureID" %in% colnames(featAnno)) {
    ribiosUtils::haltifnot(identical(as.character(featAnno[,"FeatureID"]),
                        as.character(featAnno[,1L])),
                        msg="The content in the column 'FeatureID' does not match the first column.")
  }
  if(colnames(featAnno)[1]=="") { ## the annotation file contains row names
    ...
    if(!"FeatureID" %in% colnames(featAnno)) {
      featAnno$FeatureID <- featAnno[,1]
    }
  }
  featAnno <- ribiosUtils::putColsFirst(featAnno, "FeatureID")
  featAnno$FeatureID <- as.character(featAnno$FeatureID)
  rownames(featAnno) <- as.character(featAnno[, "FeatureID"])
